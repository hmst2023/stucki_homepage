...
def delete_single_entry(request: Request, media_id:str, media_type:str, user_id=Depends(auth_handler.auth_wrapper)) -> JSONResponse:
    if not request.app.db['Users'].find_one({"_id": ObjectId(user_id)}):
