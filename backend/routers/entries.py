...
@router.post("/2")
def post_reworked(entry:PostEntry2, user_id=Depends(auth_handler.auth_wrapper), db=Depends(database)):
...
@router.post("/")
def post_new_entry(user_id=Depends(auth_handler.auth_wrapper), entry: PostEntry = Depends(),
...
def patch_entry(entry_id: str, entry:PostEntry = Depends(), user_id=Depends(auth_handler.auth_wrapper),
